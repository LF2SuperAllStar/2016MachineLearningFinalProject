...
rgs = RandomForestRegressor(n_estimators=tree_num,n_jobs=-1,warm_start=True,oob_score=True)
rgs.fit(tr_x,tr_y)

print 'predicting at', datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')
if val_size>0:
	predict_y = rgs.predict(val_x)
	...
	predict_test_y = rgs.predict(test_x)
